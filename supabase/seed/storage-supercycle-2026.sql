...
So the question "who owns HBM?" is really "who owns the AI memory profit pool?" Today, on the shipping HBM3E generation, SK hynix holds about **62%**, with Micron (~21%) having overtaken Samsung (~17%). But the 2026 battle is pivoting to **HBM4** and its allocation on NVIDIA's next-gen **Rubin** platform — where the split is expected to compress toward SK hynix ~55%, Samsung ~25%, Micron ~20%. Toggle the chart to compare:
...
*Methodology: every figure is anchored to reporting from May–June 2026 (TrendForce, Morgan Stanley via TradingKey, U.S. News, IBTimes, STCN, JRJ, Fortune, OFweek). Where a chart needs a derived value — for example a 2025 revenue-per-bit base back-calculated from a reported year-over-year change, or a price index rebased to Q4 2025 = 100 from sequential QoQ guidance — the derivation is noted in the figure's caption. Charts are rendered with D3.*
$md$,
        'Finance',
        'public');
